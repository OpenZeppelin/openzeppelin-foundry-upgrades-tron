...
/** A normalized TRON JSON-RPC endpoint pair. */
export interface NormalizedEndpoint {
  fullHost: string;
  jsonRpcEndpoint: string;
}

/** Read-only state configuration, independent of network credentials. */
...
export interface Config extends StateConfig, NormalizedEndpoint {
...
function hasNonemptyString(object: Record<string, unknown>, key: string): boolean {
  return Object.prototype.hasOwnProperty.call(object, key) && typeof object[key] === 'string' && (object[key] as string).length > 0;
}

function normalizeEndpoint(endpoint: string): NormalizedEndpoint {
...
function parseFeeLimit(value: string): number {
...
function explicitAbsolutePath(environment: NodeJS.ProcessEnv, key: string, fallback: string): string {
...
function parseStateConfig(environment: NodeJS.ProcessEnv = process.env): StateConfig {
...
function parseConfig(environment: NodeJS.ProcessEnv = process.env): Config {
  ...
  const feeLimit = Object.prototype.hasOwnProperty.call(environment, 'TRON_FEE_LIMIT')
    ? parseFeeLimit(environment.TRON_FEE_LIMIT as string)
    : DEFAULT_FEE_LIMIT;
