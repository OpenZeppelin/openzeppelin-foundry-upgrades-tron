...
import { UpstreamRpcError, createUpstreamClient } from '../../dist/rpc/upstream.js';
...
test('posts strict JSON-RPC requests through built-in fetch transport', async () => {
  ...
  const client = createUpstreamClient('http://127.0.0.1:9090/jsonrpc', {
    async fetch(endpoint, options) {
      calls.push({ endpoint: endpoint as string, options });
...
test('validates methods, params, endpoint, and fetch dependency before transport', async () => {
  ...
  assert.throws(
    () => createUpstreamClient('https://example.test', { fetch: null as unknown as typeof fetch }),
    /fetch/i,
  );
  ...
  await assert.rejects(() => client.request('eth_call', null as unknown as unknown[]), /params/i);
});
